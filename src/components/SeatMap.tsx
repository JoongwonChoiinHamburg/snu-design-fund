...
const maxSeatsPerRow = Math.max(
  ...
  return (
    <div className="space-y-10 ">
      <section>
        <h3 className="mb-5 font-bold text-lg">좌석 기부 신청 절차
        ...
        <div className="overflow-hidden bg-white">
          {STEPS.map((step, index) => (
            <div
              key={step.title}
              className="grid gap-4 border-b border-black/30 p-5 last:border-b-0 md:grid-cols-[120px_1fr]"
            >
              <div className="font-display text-[var(--color-grey)] text-2xl">
                {index + 1}
              </div>

              <div>
                <h4 className="font-display text-lg text-[var(--color-grey)] ">
                  {step.title}
                </h4>
